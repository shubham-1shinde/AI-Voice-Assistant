...
# def execute(tool_name: str, args: dict | str | None) -> str:
#     fn = TOOLS.get(tool_name)
#     if not fn:
#         return f"Unknown command: {tool_name}."

#     # Normalize args to dictionary
#     if isinstance(args, str):
#         clean_str = args.strip()
#         args = {"query": clean_str, "name": clean_str, "project": clean_str, "path": clean_str}
...
#         return fn(**args)
#     except Exception:
#         # Fallback: Pass positional value or execute parameter-less
#         try:
#             val = (
#                 args.get("query")
#                 or args.get("name")
...
#                 or ""
#             )
#             return fn(val) if val else fn()
#         except Exception as exc:
#             return f"Error executing {tool_name}: {exc}"

from typing import Callable

from . import browser, media, system_info, vscode, whatsapp, windows_ctl
...
def tool_names() -> list[str]:
    """Returns a list of all registered tool names for the planner prompt."""
    return list(TOOLS.keys())
...
def execute(tool_name: str, args: dict | str | None) -> str:
    ...
    # Standardize args if passed as string or None
    if isinstance(args, str):
        clean_str = args.strip()
        args = {"query": clean_str, "name": clean_str, "project": clean_str, "path": clean_str}
    elif not isinstance(args, dict):
    ...
    try:
        ...
        # Fallback if function doesn't accept **kwargs or expects a single positional arg
        try:
            val = (
                args.get("query")
                or args.get("name")
